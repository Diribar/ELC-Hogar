...
const FN_resultados = {
	obtiene: async function () {
		...
		const APIs = [{ruta: "obtiene-los-resultados/?datos=" + JSON.stringify(prefs), duracion: 500}]; // 'duración' son los milisegs que se estima que puede durar el fetch
	...
	muestra: function () {
		...
		v.layoutBD.boton ? botones.general() : listados.general();
...
const botones = {
	general: function () {
		...
		if (v.entidad == "productos" && v.layoutBD.boton) {
	...
	boton: function (registro) {
		...
		if (familia == "rclv") {
			let prodsCons = [];
			for (let producto of registro.productos) prodsCons.push([producto.entidad, producto.id]);
...
const listados = {
	general: function () {
		...
		v.resultados.forEach((registro, indice) => {
			...
			if (titulo) {
				DOM.tabla = this.creaUnaTabla({titulo, indice}); // Obtiene la tabla con los datos
			...
			if (v.entidad == "productos") {
				const fila = this.creaUnaFilaDeProd({producto: registro, indice});
	...
	creaUnaFilaDeProd: function ({producto, indice}) {
		...
		celda = this.creaUnaCelda.prod(producto, vistaSinCalificar);
		...
		if (["calificacion", "misCalificadas"].includes(v.layoutBD.codigo) && producto.calificacion) {
			celda = this.creaUnaCelda.calificacion(producto);
			fila.appendChild(celda);
		}
	...
	creaUnaCelda: {
		rclv: (rclv) => {
			...
			if (v.layoutBD.codigo == "santoralLargo") segundaLinea.innerHTML += rclv.fechaDelAno; // Día del Año
