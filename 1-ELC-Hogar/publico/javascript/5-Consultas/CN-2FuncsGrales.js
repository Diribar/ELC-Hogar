...
const actualiza = {
	...
	botoneraActivaInactiva: () => {
		...
		// Sin filtros - casos en los cuales activar el ícono
		if (
			v.camposFiltros.some(
				(campo) =>
					DOM[campo].value && // tiene un valor
					DOM[campo].value != "todos" && // distinto a 'todos'
					!DOM[campo].parentNode.className.includes("ocultar") // está visible
			)
		) {
			DOM.sinFiltros.classList.remove("inactivo");
			DOM.sinFiltros.title = v.titulo.sinFiltros;
		}
		// Sin filtros - casos en los cuales inactivar el ícono
		else {
			DOM.sinFiltros.classList.add("inactivo");
			DOM.sinFiltros.title = "No hay filtros aplicados";
		}
...
const accionesPorCambioDePrefs = async () => {
	...
	if (v.layout_id) {
		...
		if (v.mostrarResultados) FN_resultados.muestra(); // muestra los resultados
	}
