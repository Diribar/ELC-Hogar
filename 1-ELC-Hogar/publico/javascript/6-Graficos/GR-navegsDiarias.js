...
window.addEventListener("load", async () => {
	...
	const navegsDiarias = await fetch(ruta).then((n) => n.json());
	...
	for (let navegsDelDia of navegsDiarias) {
		// Alimenta los datos del gráfico
		const {fecha, anoMes, logins, usSinLogin, visitas} = navegsDelDia;
	...
	const cantConFecha = navegsDiarias.filter((n) => n.fecha).length;
