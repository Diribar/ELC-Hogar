...
module.exports = {
	...
	rutinas: {
		...
		actualizaProdsAlAzar: async () => {
			...
			const prodsAzar = await baseDeDatos.obtieneTodos("prodsAzar");
			...
			for (let entidad of entidades) {
				...
				for (let producto of productos) {
					...
					// Guarda los cambios - actualiza el campo 'azar'
					if (prodsAzar.length && prodsAzar[0].id == id) {
						baseDeDatos.actualizaPorCondicion("prodsAzar", {id}, datos);
						prodsAzar.shift();
					}
					// Guarda los cambios - agrega un registro
					else baseDeDatos.agregaRegistro("prodsAzar", {id, ...datos}); // agrega un registro
				}
			}

			// Elimina los sobrantes
			baseDeDatos.eliminaPorCondicion("prodsAzar", {id: {[Op.gt]: id}});
		...
		navegsDia: async () => {
			...
			// Procesos
			espera.push(procesos.navegsDia.navegsDiaRuta(navegsDia));
			espera.push(procesos.navegsDia.navegsDiaProd(navegsDia));
			espera.push(procesos.navegsDia.navegsDiaHora(navegsDia));
