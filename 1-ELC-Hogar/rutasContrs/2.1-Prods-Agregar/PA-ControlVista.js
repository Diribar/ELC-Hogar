...
module.exports = {
	...
	datosDuros: {
		form: async (req, res) => {
			...
			const errores = await validacsFM.datosDuros(camposDD_nombre, datosDuros);
		...
		guardar: async (req, res) => {
			...
			let errores = await validacsFM.datosDuros(camposDD_nombre, datosDuros);
			if (errores.hay) return res.redirect(req.originalUrl);
	...
	datosAdics: {
		...
		guardar: async (req, res) => {
			...
			let errores = await validacsFM.datosAdics(camposDA, datosAdics);
			if (errores.hay) return res.redirect(req.originalUrl);
	...
	confirma: {
		form: (req, res) => {
			...
			// Imagen derecha
			const imgDerPers = !confirma.avatar ? confirma.avatarUrl : "/Externa/9-Provisorio/" + confirma.avatar;
		...
		guardar: async (req, res) => {
			...
			if (!confirma.avatar) {
				...
				const rutaYnombre = carpetaExterna + "2-Productos/Revisar/" + confirma.avatar;
	...
	FA: {
		...
		guardar: async (req, res) => {
			...
			// Acciones si es un capítulo
			if (datosDuros.entidad == "capitulos") return accionesParaCapitulosIMFA(datosDuros, req, res);
