...
module.exports = {
	detalle: async (req, res) => {
		...
		const prodsDelRclv = await procesos.prodsDelRclv
			.agregaProdsDesdeEdicsRclv(rclv, usuario_id) // agrega productos
			.then((rclv) => procesos.prodsDelRclv.consolidaLosProds(rclv, usuario_id)) // consolida los productos
			.then(async (prods) => await procesos.prodsDelRclv.agregaInfoALosProds(prods, usuario_id)) // enriquece los productos
			.then((prods) => procesos.prodsDelRclv.fueraDeConsulta(prods, prodsCons));
