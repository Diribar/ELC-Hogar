		...
		prodsAsocs: async (entidad, id) => {
			...
			for (let entProd of entsProd) {
				...
				for (let prodVinculado of prodsVinculados) {
					...
					const errores = await validacsFM.consolidado({datos: prodVinculado});
...
let FN = {
	...
	statusFinalMasMotivo: async ({codigo, desaprueba, rclv, entidad, original, req}) => {
		...
		const impideAprobado = statusAprob && !rclv ? await validacsFM.consolidado({datos}).then((n) => n.impideAprobado) : null;
