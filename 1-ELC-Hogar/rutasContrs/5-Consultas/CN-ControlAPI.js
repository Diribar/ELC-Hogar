...
const layoutDefault_id = 13; // El 'default' es "Nuestra propuesta"
...
module.exports = {
	obtiene: {
		...
		preferencias: async (req, res) => {
			...
			// Correcciones
			if (!preferencias || !preferencias.layout_id) preferencias = {layout_id: layoutDefault_id};
			else {
				// Miscelaneas
				...
				if (!usuario_id && layout_id) {
					...
					if (layout && layout.grupoSelect == "loginNeces") preferencias.layout_id = layoutDefault_id;
				}
			}
	...
	resultados: async (req, res) => {
		...
		if (entidad == "productos") {
			...
			prods = procesos.resultados.descartaCapsSiColPresente.prods(prods); // tiene que estar último
			prods = procesos.resultados.orden.prods({prods, layout}); // Ordena los productos
			prods = procesos.resultados.quitaExcedente({resultados: prods, layout, prefs});
