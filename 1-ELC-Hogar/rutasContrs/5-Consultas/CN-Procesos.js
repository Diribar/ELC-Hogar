	...
	resultados: {
		obtieneProds: {
					...
					productos.push(
						baseDeDatos.obtieneTodosPorCondicion(entProd, condicion, include).then((prods) =>
							prods.map((prod) => {
								...
								if (fechaHoy) respuesta.hoy = true;
			...
			filtrosNoAplicablesEnBD: ({productos, prefs, campo_id}) => {
				...
				// cfc / vpc
				if (cfc) productos = productos.filter((n) => (cfc == "1" ? n.cfc : !n.cfc)); // si no es 'true', incluye los 'null'
		...
		obtieneRclvs: {
			...
			porFechaDelAno: async (prefs) => {
				...
				for (let entRclv of entsRclv) {
					...
					const includes = entRclv == "personajes" ? inclPers : entRclv == "hechos" ? inclHec : inclStd;
		...
		orden: {
			prods: ({prods, layout}) => {
				...
				if (["santoralAzar", "azar"].includes(layout.codigo))
					prods.sort((a, b) =>
						a.prodAzar && b.prodAzar ? a.prodAzar.azar - b.prodAzar.azar : b.prodAzar.azar ? -1 : 0
					);
				// Resto
		...
		camposNecesarios: {
			prods: (prods) => {
				...
				prods = prods.map((prod) => {
					...
					const {entidad, id, nombreCastellano, ppp, avatar, cfc, linksGral} = prod;
					const datosNeces = {entidad, id, nombreCastellano, ppp, avatar, cfc, linksGral};
...
const alAzar = {
	consolidado: function ({resultados, cantResults, prefs, layout}) {
		...
		// 1. Elije por motivos prioritarios
		...
		while (v.contador < 4 && v.resultados.length) {
			...
			v.resultado = v.resultados.find(
				(n) =>
					...
					(n.cfc && v.cfc <= v.vpc) || // se debe equilibrar y no se alcanzaron los 2 resultados 'cfc'
					(!n.cfc && v.vpc <= v.cfc) // se debe equilibrar y no se alcanzaron los 2 resultados 'vpc'
			);
		...
		if (layout.codigo == "santoralAzar") v.productos.sort((a, b) => (a.hoy === b.hoy ? 0 : a.hoy ? -1 : 1)); // prioriza los productos del día
